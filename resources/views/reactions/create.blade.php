...
@section('content')
    <div class="question">
        ...
        <div class="question-answer">
            <form action="{{ route('reaction.store',$question->id) }}" method="POST">
                @csrf
                <div>
                    <label for="text">Reaction</label>
                    <textarea name="name" id="name" rows="5" class="form-control"
                        placeholder="Enter your reaction"></textarea>
                </div>
                <input type="hidden" name="question_id" value="{{ $question->id }}">
                <div><input type="submit" value="Submit Your Reaction" class="btn btn-primary"></div>
            </form>
        </div>
        {{-- <div class="question-answer">
